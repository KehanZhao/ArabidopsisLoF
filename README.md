...
Paper: Zhao et al., *The Plant Cell*, 2024 — https://doi.org/10.1093/plcell/koag087
...
The pipeline proceeds in four stages: (1) LoF variant calling from two independent variant sources, (2) expression matrix construction aligned to the LoF matrix, (3) EMMAX-based burden–expression association testing, and (4) network visualization of significant associations.
...
> **Note:** `LoF_calling.R` is specifically tailored for the Arabidopsis TAIR10 annotation and the datasets described in the paper. For a species-agnostic LoF caller, see [LoFMatrixBuilder](https://github.com/KehanZhao/LoFMatrixBuilder).
...
Prior to testing, expression genes with zero variance are dropped, LoF genes with fewer than 3 alleles across accessions are excluded, and both matrices are subsetted to their shared accessions. An inverse normal transformation (INT) function is included but not applied by default; the relevant line is commented in the code.
...
Builds and plots a directed gene network from significant LoF–expression associations. Edges point from LoF gene (source, circle node) to expression gene (target, square node) and are colored by direction of effect: pink for activation (positive β) and blue for repression (negative β).

The network is filtered to LoF genes with ≥10 significant expression associations. Highlighted genes — by default, all expression targets of *FRI* (AT4G00650) plus FRI itself — are rendered in orange with labels; all other nodes are small and unlabeled. Two plots are written to a single PDF: one with labels on highlighted genes and one without.
...
> Zhao K, *et al.* Loss-of-function burden–gene expression associations in *Arabidopsis thaliana*. *The Plant Cell*, 2024. https://doi.org/10.1093/plcell/koag087
