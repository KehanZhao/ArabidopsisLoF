# LoF_calling.R
# Identifies loss-of-function (LoF) variants in Arabidopsis thaliana from two
# sources: (1) structural variants (SVs/deletions) from the European Variation
# Archive, and (2) high-impact SNPs/indels from the 1001 Genomes Project.
# The two resulting binary LoF matrices are merged into a single collapsed matrix.
#
# NOTE: This script is specific to the Arabidopsis TAIR10 annotation and the
# datasets described in the paper. For a species-agnostic LoF caller, see:
# https://github.com/KehanZhao/LoFMatrixBuilder
...
# =============================================================================
# 1. SHARED ANNOTATION: Load TAIR10 gene/CDS models
# =============================================================================

# Read GFF3 annotation and assign column names
gff <- fread("/path/to/TAIR10_GFF3_genes.gff")
colnames(gff) <- c("chr", "source", "type", "start", "stop", "X", "direction", "X2", "info")

# Subset to CDS regions (only deletions overlapping coding sequence are called)
CDS <- gff[type == "CDS"]
CDS$chr <- gsub("Chr", "", CDS$chr)  # Strip "Chr" prefix to match VCF chromosome names

# Extract gene model and gene name from the GFF3 info column
CDS$model <- gsub("Parent=(.+),.+", "\\1", CDS$info)
CDS$gene  <- gsub("\\..+", "", CDS$model)
CDS_split <- split(CDS, by = "chr")  # Pre-split by chromosome for fast lookup

# Subset to gene-level records
gene <- gff[type == "gene"]
gene$chr   <- gsub("Chr", "", gene$chr)
gene$model <- gsub("ID=(.+);.+", "\\1", gene$info)

# Load full gene model list (used for affected-model percentage calculation)
All_gene_model <- read.delim("/path/to/TAIR10_all_gene_models.txt")

# =============================================================================
# 2. LOF FROM STRUCTURAL VARIANTS (DELETIONS)
# =============================================================================
...
# Clean genotype calls: extract allele dosage, treat missing (".") as reference (0)
...
# Identify LoF variants: frameshift and/or large deletion (>10% of CDS) --------
# For each variant, we:
#   - Find all CDS models overlapping the deletion interval
#   - Restrict to the 5–95% CDS middle region to avoid annotation artefacts at
#     UTR boundaries
#   - Flag as frameshift if deletion length is not divisible by 3
#   - Flag as large deletion if >10% of the CDS is affected
...
for (i in seq_len(nrow(variants))) {
  ...
  real_lof_new  <- character(0)
  frameshift    <- character(0)
  large_deletion <- character(0)

  for (candidate in overlapping_models) {
    gene_CDS     <- sort(unlist(apply(CDS[model == candidate], 1, function(x) x[4]:x[5])))
    n            <- length(gene_CDS)
    middle_region <- gene_CDS[-c(1:floor(0.05 * n), floor(0.95 * n):n)]
  ...
  lof_new_vector[i]        <- if (length(real_lof_new)   > 0) paste(real_lof_new,   collapse = ",") else NA
  frameshift_vector[i]     <- if (length(frameshift)      > 0) paste(frameshift,      collapse = ",") else NA
  large_deletion_vector[i] <- if (length(large_deletion) > 0) paste(large_deletion, collapse = ",") else NA

  setTxtProgressBar(pb, i)
}
close(pb)

# Attach LoF annotation columns to variants table
...
# Extract unique gene names from model names (first 9 characters = gene ID)
...
# Calculate affected model counts and percentages -------------------------------
...
for (i in seq_len(nrow(variants))) {
  ...
  if (!is.na(gene)) {
    ...
    model_counts    <- sapply(gene_list, function(g) sum(grepl(g, All_gene_model[[1]])))
    affected_counts <- sapply(gene_list, function(g) str_count(affected_models, g))
...
# Calculate percentage of CDS length affected ----------------------------------
...
# =============================================================================
# 3. LOF FROM 1001 GENOMES HIGH-IMPACT VARIANTS
# =============================================================================

# High-impact variants were pre-extracted from the full VCF with:
#   zgrep "HIGH" 1001genomes_snp-short-indel_only_ACGTN_v3.1.vcf.snpeff.gz > high_impact.txt

high_impact <- read.delim("/path/to/high_impact.txt", header = FALSE)
header      <- read.delim("/path/to/1001GenomeVCF_header.txt", header = FALSE)
colnames(high_impact) <- header$V1

# Split info columns from genotype columns
high_impact_info <- high_impact[, 1:9]
high_impact_gt   <- high_impact[, -c(1:9)]

# Clean genotype calls: extract allele dosage, treat missing (".") as ref (0)
...
# Expand multi-allelic sites: create one row per alternate allele ---------------
...
for (i in seq_along(multiple_changes)) {
  ...
  # Update original row: restrict to first allele only
...
# Assign gene IDs (first 9 characters of model names)
...
# Calculate affected model percentages -----------------------------------------
...
# Determine LoF type from SnpEff annotation ------------------------------------
...
# Determine gene model restricted to 5–95% of CDS length ----------------------
# Same 5–95% middle-region filter applied to SVs above.
...
# =============================================================================
# 4. BUILD LOF MATRICES
# =============================================================================

# LoF matrix from structural variants (deletions) ------------------------------
# Columns = genes; rows = accessions; value = LoF allele dosage (binarized to 0/1 later)
...
LoF_matrix_SV <- lapply(unique(CDS$gene), function(g) {
  ...
  if (length(lof_var_index) > 0) {
    ...
    # For variants annotated to multiple genes, extract the entry for gene g
    ...
    # Keep only variants that affect all models of the gene
...
LoF_matrix_SV           <- as.matrix(do.call(cbind, LoF_matrix_SV))
rownames(LoF_matrix_SV) <- colnames(gt_processed)

# LoF matrix from 1001 Genomes high-impact SNPs/indels -------------------------
...
LoF_matrix_1001 <- lapply(unique(CDS$gene), function(g) {
  ...
  if (length(lof_var_index) > 0) {
    ...
    # For multi-gene variants, extract the entry for gene g
    multiple_genes <- which(grepl(",", info_subset$gene))
    if (length(multiple_genes) > 0) {
      for (i in multiple_genes) {
        genes    <- strsplit(info_subset$gene[i], ",")[[1]]
    ...
    # Filter 1: remove variants not affecting all gene models
    delete1 <- which(as.numeric(info_subset$affected_percentage) != 1)

    # Filter 2: frameshifts/stop_gains outside the 5–95% CDS region
    delete2 <- vector()
    for (o in seq_len(nrow(info_subset))) {
      lof_type <- paste(unique(strsplit(info_subset$LoF_type[o], ",")[[1]]), collapse = ",")
      if (lof_type %in% c("frameshift_variant", "stop_gained") &&
          info_subset$gene_model_5_95[o] == "NA") {
        delete2 <- c(delete2, o)
      }
    }

    # Filter 3: unannotated LoF type
...
LoF_matrix_1001           <- as.matrix(do.call(cbind, LoF_matrix_1001))
rownames(LoF_matrix_1001) <- colnames(high_impact_gt_processed)

# =============================================================================
# 5. COLLAPSE THE TWO LOF MATRICES
# =============================================================================

# Sanity checks
stopifnot(identical(colnames(LoF_matrix_1001), colnames(LoF_matrix_SV)))

Accession_list_intersect <- intersect(rownames(LoF_matrix_1001), rownames(LoF_matrix_SV))

Matrix_1_intersect <- LoF_matrix_1001[Accession_list_intersect, ]
Matrix_2_intersect <- LoF_matrix_SV[Accession_list_intersect, ]

stopifnot(identical(colnames(Matrix_1_intersect),  colnames(Matrix_2_intersect)))
stopifnot(identical(rownames(Matrix_1_intersect), rownames(Matrix_2_intersect)))

# Sum matrices then binarize (LoF presence/absence)
